...
import { downloadMigrationTemplate } from "@/lib/generate-migration-template";
import { autoMapColumns, DOORSTAX_FIELDS } from "@/lib/migration-presets";
...
const DOORSTAX_FIELD_KEYS = DOORSTAX_FIELDS.map((f) => f.key);
...
export default function MigrateTenantsPage() {
  ...
  useEffect(() => {
    fetch("/api/tenants/migrate/presets")
      .then((r) => r.json())
      .then(setPresets)
      .catch(() => {});
  }, []);
  ...
  function selectPlatform(id: string) {
    ...
    // Dynamically import the full preset for column aliases
    import("@/lib/migration-presets").then((mod) => {
      const preset = mod.MIGRATION_PRESETS.find((p) => p.id === id);
  ...
  function processRawData(headers: string[], rows: Record<string, string>[]) {
    ...
    if (activePreset) {
      ...
      for (const h of headers) {
        if ((DOORSTAX_FIELD_KEYS as string[]).includes(h.toLowerCase().trim())) {
  ...
  function applyMapping() {
    ...
    for (let i = 0; i < rawRows.length; i++) {
      ...
      if (!row.email || !emailRegex.test(row.email)) { errs.push(`Row ${rowNum}: Invalid email "${row.email || ""}"`); continue; }
    ...
    for (const row of mapped) {
      ...
      let unit = prop.units.find(
        (u) => u.unitNumber.toLowerCase() === row.unitNumber.toLowerCase()
      );
      if (!unit) {
        unit = { unitNumber: row.unitNumber, tenants: [] };
        prop.units.push(unit);
      }
      unit.tenants.push(row);
    }
  ...
  async function runImport() {
    setImporting(true);
    try {
      const res = await fetch("/api/tenants/migrate", {
        method: "POST",
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Import properties, units, and tenants from another platform
          </p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border bg-muted/30 p-4">
            ...
            >
              ...
              Download blank migration template
            </button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div className="rounded-xl border bg-card p-4 text-center">
              ...
              <p className="text-xl font-bold">
                {parsedRows.filter((r) => r.leaseStart || r.leaseEnd).length}
              </p>
            ...
            {grouped.map((prop) => (
              ...
              >
                <div className="px-4 py-3 bg-muted/50 border-b">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-auto text-xs text-muted-foreground">
                      ...
                      {prop.units.reduce((s, u) => s + u.tenants.length, 0)}{" "}
                      tenant(s)
                    </span>
                  </div>
                </div>
                <div className="divide-y">
                  {prop.units.map((unit) => (
                    <div key={unit.unitNumber} className="px-4 py-2">
          ...
          <div className="flex gap-3">
            ...
            >
              {importing ? (
                ...
                `Import ${parsedRows.length} Tenant(s)`
              )}
            </button>
          </div>
        </div>
      ...
      {step === "result" && result && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-5 gap-4">
            {[
              { label: "Properties", value: result.propertiesCreated },
              { label: "Units", value: result.unitsCreated },
              { label: "Tenants", value: result.tenantsCreated },
              { label: "Leases", value: result.leasesCreated },
              { label: "Skipped", value: result.tenantsSkipped },
            ].map((item) => (
              <div
                key={item.label}
