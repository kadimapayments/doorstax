...
export async function GET(req: NextRequest) {
  try {
    ...
    const report = await generateProfitAndLoss({ pmId: session.user.id, startDate, endDate, propertyId, ownerId });
    return NextResponse.json(report);
  } catch (err) {
    console.error("[accounting/reports/profit-loss] error:", err);
